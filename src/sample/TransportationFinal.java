...
import sample.ResultFrame.ResultController;
...
public class TransportationFinal {

    private static int[] demand;
    private static int[] supply;
    private static double[][] costs;
    private static Shipment[][] matrix;
    private static double totalCosts;
    private static String[][] marixFinal1;
    private static String marixFinal;
    ...
    static void init(){
        ...
        List<Integer> dst = ResultController.getDemandFinal();
    ...
    //Печатаем результат оптимальный план перевозок и минимальные затраты
    static void printResult() {

        totalCosts = 0;
        marixFinal1 = new String[supply.length][demand.length];
        marixFinal = "При данных потребностях определяем оптимальный план перевозок: \n";
        for (int r = 0; r < supply.length; r++) {
            for (int c = 0; c < demand.length-1; c++) {
                Shipment s = matrix[r][c];
                if (s != null && s.r == r && s.c == c) {
                    marixFinal1[r][c] = String.format(" %3s ", (int) s.quantity);
                    marixFinal +="X" + (r+1) + (c+1) + " = " + String.format("%3s", (int) s.quantity) + ", ";
        ...
        marixFinal += String.format("При этом минимальные затраты равны %.2f", totalCosts);
    }
    ...
    public static String getMatrixFinal(){
        return marixFinal;
    }
}
